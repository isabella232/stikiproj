...
class AbsenDosen{
    ...
    public function listabsen($TGL,$pg='',$dsn=''){
        ...
        if(strlen($dsn)>5){
            $sql1 = "SELECT DISTINCT a.IDDOSEN, a.DOSEN, o.TGL, o.JAM1, o.JAM2, o.STTHDR, o.KET, o.IDJADWAL,j.MULAIJAMKE,j.BERAKHIRJAMKE, j.IDMK, o.idaksesndosen  FROM dp as a left outer join absensidosen as o on a.IDDOSEN = o.IDDOSEN inner join jadwal as j on j.IDJADWAL = o.IDJADWAL WHERE a.DOSEN ='" . $dsn . "' a.HARI='" . $GRH . "' and o.TGL='" . $TGL . "' limit $position,10;";
        }
        $stmt = mysql_query($sql1,$this->conn);
    ...
    public function rekapabsenlist($pg,$tgl1,$tgl2,$filter){
        ...
        //$sql1 = "SELECT IDDOSEN, DOSEN, NIDN FROM dosen limit 0,10;";
        $stmt = mysql_query($sql1,$this->conn);
    ...
    public function lapdosendetail($idosen,$tgl1,$tgl2){
        ...
        $sql1 = "SELECT DISTINCT	a.TGL, a.HARI, a.STTHDR, a.KET, d.DOSEN, d.NIDN, j.IDMK FROM absensidosen as a inner join dosen as d on a.IDDOSEN=d.IDDOSEN inner join jadwal as j on a.IDJADWAL=j.IDJADWAL where a.IDDOSEN=$idosen and tgl BETWEEN '" . $tgl1 . "' and '" . $tgl2 . "' order by IDMK;";
        $stmt = mysql_query($sql1,$this->conn);
        
        $tx = "<div class='headerdl'></div>";
        $tx .= "<table id=\"rounded-corner\" class='daftardosen'>";
        $tx .= "<thead><tr><th>No</th><th>TGL</th><th>MATAKULIAH</th><th>SKS</th><th>HADIR</th></thead><tbody>";
